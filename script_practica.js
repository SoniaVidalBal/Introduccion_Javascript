...
async function whattodo() {
  ...
  do {
    ...
    if (numberconsole == 0 || numberconsole > 18) {
      rl.close();
      console.log("Hasta pronto!");
    } else {
      orders(numberconsole);
    }
  } while (numberconsole != 0 && numberconsole <= 18);
}
...
function orders(numberconsole) {
  switch (numberconsole) {
    ...
    case 9 /* Mostrar los nombres de los alumnos que tengan entre 20 y 25 años */:
      const intwenties = students.filter(
    ...
    case 12 /* Mostrar edad media de todos los alumnos */:
      const sumAge =
        students.reduce((accum, students) => accum + students.age, 0) /
        students.length;
      console.log(sumAge);
      break;
    case 13 /* Mostrar la edad media de las chicas */:
      ...
      const sum =
        newfemlist.reduce((accum, girl) => accum + girl, 0) / newfemlist.length;
      console.log(sum);
      break;
    ...
    case 15 /* Ordenar la lista de alumnos alfabéticamente */:
      console.table(
        students.sort(function (alumno1, alumno2) {
    ...
    case 16 /* Mostrar por consola el alumno con mejores notas. Sumatorio de notas más alto */:
      ...
      students.forEach((student) =>
        sumatorio.push(
          student.examScores.reduce((accum, nota) => accum + nota, 0)
        )
      );
      const max = Math.max(...sumatorio);
      for (let index = 0; index < students.length; index++) {
        const suma = students[index].examScores.reduce(
          (accum, nota) => accum + nota,
          0
        );
        if (suma === max) {
    ...
    case 17 /* Mostrar la nota media más alta de la clase y el nombre del alumno al que pertenece */:
      ...
      students.forEach((student) =>
        notamedia.push(
          student.examScores.reduce((accum, nota) => accum + nota, 0) /
            student.examScores.length
        )
      );
      const maxnota = Math.max(...notamedia);
      for (let index = 0; index < students.length; index++) {
        const media =
          students[index].examScores.reduce((accum, nota) => accum + nota, 0) /
    ...
    case 18 /*Añadir un punto extra a cada nota existente de todos los alumnos. La nota máxima posible es 10. 
      ...
      for (let index = 0; index < students.length; index++) {
        students[index].examScores = students[index].examScores.map(function (
          x
        ) {
          if (x < 10) {
            return (x += 1);
